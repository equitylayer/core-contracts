...
contract SharedHonkVerifier {
    ...
    /// @notice Verify a Honk proof against a per-circuit verification key.
    /// @dev    Per-circuit constants (logN, vkHash, numPublicInputs) are passed in by the
    ///         per-circuit wrapper rather than baked in as immutables, so a single deployment
    ///         of this contract serves all circuits and stays under the EIP-170 size limit.
    function verify(
        ...
        uint256 msmSize = NUMBER_UNSHIFTED_ZK + logN + LIBRA_COMMITMENTS + 2;

        // Calculate expected proof size based on logN
        uint256 expectedProofSize = calculateProofSize(logN);

        // Check the received proof is the expected size where each field element is 32 bytes
        require(
            proof.length == expectedProofSize, Errors.ProofLengthWrongWithLogN(logN, proof.length, expectedProofSize)
        );

        Honk.ZKProof memory p = ZKTranscriptLib.loadProof(proof, logN);

        require(publicInputs.length == vk.publicInputsSize - PAIRING_POINTS_SIZE, Errors.PublicInputsLengthWrong());

        // Generate the fiat shamir challenges for the whole protocol
        ZKTranscript memory t = ZKTranscriptLib.generateTranscript(p, publicInputs, vkHash, numPublicInputs, logN);
        ...
        // Sumcheck
        require(verifySumcheck(p, t, logN), Errors.SumcheckFailed());
        require(verifyShplemini(p, vk, t, logN, msmSize), Errors.ShpleminiFailed());
    ...
    function verifySumcheck(Honk.ZKProof memory proof, ZKTranscript memory tp, uint256 logN)
        internal
        view
    ...
    function verifyShplemini(
        ...
        Fr[] memory powers_of_evaluation_challenge = CommitmentSchemeLib.computeSquares(tp.geminiR, logN);
        ...
        * 3. Accumulates the summands of the constant term:
        ...
        require(
            checkEvalsConsistency(
                proof.libraPolyEvals, tp.geminiR, tp.sumCheckUChallenges, proof.libraEvaluation, logN
            ),
        ...
        pair.P_0 = batchMul(commitments, scalars, msmSize);
    ...
    function batchMul(Honk.G1Point[] memory base, Fr[] memory scalars, uint256 msmSize)
        internal
        view
